...
def windpath_predict(predictDateTime, predictCoordinates):
    ...
    # cmap용 정규분포 개수 확인 데이터프레임 생성 -> Tạo khung dữ liệu để kiểm tra số lượng phân phối chuẩn
    ...
    print("sensor_grid['lower_left_lot']: ", sensor_grid['lower_left_lot'])
    print("sensor_grid['lower_left_lat']: ", sensor_grid['lower_left_lat'])
    # print("width: ", width)
    ...
    for c in range(t):
        ...
        for a in range(normal_num):
            if (c == 0):  # t-1인 경우 정규 분포 모두 같은 위치에서 시작 -> Nếu cả phân phối chuẩn đều bắt đầu ở cùng một nơi
...
resultofLong = windpath_predict('2023-04-11 11:28:00',[129.32739, 35.456574])
print(resultofLong)
