...
// const baseUrl = '172.17.101.225:5001'
const baseUrl = 'http://192.168.0.140:5001';
...
async function loadDataAndInitMap() {
    try {
        // 데이터 로드 비동기 작업 시작 Start a data load asynchronous operation
        let response = await fetch(`${baseUrl}/setting_grid_code`, { method: 'POST' });
		// let response = await fetch(`http://203.250.78.213:5001/setting_grid_code`, { method: 'POST' });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('mySlider').addEventListener('input', function(e) {
		...
		// tag with mySlider id is selected
        let slider = e.target;
		console.log("slider = ", slider);
	    // Disable the slider
	    slider.disabled = true;
    	console.log("Slider value I selected: ", slider.value);
    	
	    // Get the value of the current slider
	    let currentValue = parseInt(slider.value);
		
		// slider.max giá trị lớn nhất
		//if (none_location == false){
	    // 처음에 왼쪽으로 슬라이드 시 한 칸씩만 **** When sliding to the left at first, only one space at a time
	...
	google.maps.event.addListener(map, 'click', function(event) {
		...
		promise.then(() => {
			...
			setTimeout(() => {
				...
				var clickedLocation = event.latLng; // Tọa độ của vị trí mà người dùng click

				// lat and lat of choosing point
				lngPoint = clickedLocation.lng();
				latPoint = clickedLocation.lat()
				choosingPoint = {lngPoint, latPoint}

				// Time 

				dateofbirth = document.getElementById("datePicker").value;
				hours = document.getElementById("timePicker").value;

				time = { dateofbirth, hours }

				// Tính toán khoảng cách từ vị trí click đến các điểm khác
				var distances = [];
				for (var i = 0; i < deCoordinate.length; i++) {
					var diemHienTai = new google.maps.LatLng(deCoordinate[i].lat, deCoordinate[i].lng);
					var distance = google.maps.geometry.spherical.computeDistanceBetween(clickedLocation, diemHienTai);
					distances.push({ id: deCoordinate[i].id, distance: distance });
				}
			
				// Sắp xếp các khoảng cách để lấy 3 điểm gần nhất
				distances.sort(function(a, b) {
					return a.distance - b.distance;
				});
			
				// get 3 nearest point
				var nearestPoints = distances.slice(0, 3);

				// showing id, name, lat, lng of 3 nearest point
				var nearestIDs = nearestPoints.map(function(point) {

					return point.id

				});

				var dataof3NearestPoint = [];
				var NearestPoint3 = [];

				for (let index = 0; index < deCoordinate.length; index++) {
					for (let points = 0; points < nearestPoints.length; points++) {
						const nearpoint = nearestPoints[points];
						if(deCoordinate[index].id == nearpoint.id){
							dataof3NearestPoint = {
								id: deCoordinate[index].id,
								name: deCoordinate[index].name,
								lng: deCoordinate[index].lng,
								lat: deCoordinate[index].lat,
							}

							// information of 3 nearest points
							NearestPoint3.push(dataof3NearestPoint)
						}
					}
				}

				data = {choosingPoint, time, NearestPoint3};

				console.log("3 điểm gần nhất: "+JSON.stringify(data));
				
				fetch(`${baseUrl}/calculate_vitual_sensor`, {
		            method: 'POST',
		            headers: {
					...
					console.log("show data calculate sensor", data);
				}).catch((error) => {
		...
		fetch(`${baseUrl}/get_sensor_location`, { method: 'POST' }) 
		    ...
		    .then(data => {
				...
				for(let i = 0; i < sensor_name.length; i++){
					...
					deCoordinate.push({ id: s_num, lat: s_lat, lng: s_lot, name: `WT${s_num}`})
                    ...
                    // 텍스트 오버레이 추가 add background for text
...
function date_time_search() {
	...
	*/
	...
	searchButton.onclick = function() {
	    ...
	    if (date && time) {
	        if (selectedCheckbox) { //관심지점 체크박스 선택 시 -> When checking the point of interest checkbox
	            none_sliderLegend();
...
async function run_predict(date, time, selectedCoordinates) {
    ...
    try {
		...
		console.log("exist_dateTime:", exist_dateTime)
		console.log("exist_before:", exist_before)
		console.log("predictCoordinates: ", predictCoordinates);		

		if(exist_dateTime > 0 && exist_before > 0 && predictCoordinates != null){
		    console.log("predictDateTime: ", predictDateTime);
		    console.log("predictCoordinates: ", predictCoordinates);
			...
			response1 = await fetch(`${baseUrl}/windpath_predict_code`, {
			    method: 'POST',
			    headers: {
			        'Content-Type': 'application/json'
			    },
...
async function usingSlider(predict_t_n){
	...
	// console.log("predict_polylines = ", predict_polylines);
	...
	if (none_location == true) {
		...
		data = await response.json();
		console.log("data trường hợp 1: ", data);
			...
			for (let k = 0; k < no_m_name[predict_t_n].length; k++) {
				...
				if (condition2(k)) {
					...
					if (!isNaN(x) && !isNaN(y)) {
						...
						addMarkerWithInfo(latitude, longitude, speed, direction, map, p_m_name2);
						let polyline = new google.maps.Polyline(polylineOptions);
						polyline.setMap(map);
		...
		try {
			...
			let data = await response.json();
			console.log("data sensor count information", data);
			...
			data = await response.json();
			console.log("sensor grid information: ", data);
			...
			// return Returns predicted data around a specific time
			...
			data = await response.json();
			console.log("data of estimate_grid_sensor_data: ", data);
			...
			// 풍속/풍향 그리기 -> Drawing wind speed/direction -> hàm này dùng để vẽ hướng gió và tốc độ gió

			console.log("p_all_grid_num: ", p_all_grid_num);

			// p_all_grid_num biến này để làm gì?
			...
			for (let k = 0; k < p_all_grid_num; k++) {
				if (condition2(k)) {
					...
					console.log("vector_x: ", vector_x);
					console.log("vector_y: ", vector_y);
					...
					if (!isNaN(x) && !isNaN(y)) {
						...
						addMarkerWithInfo(latitude, longitude, speed, direction, map, p_m_id2[k]);
						let polyline = new google.maps.Polyline(polylineOptions);
						polyline.setMap(map);
						polylines.push(polyline);
					}
...
let contentString;
function addMarkerWithInfo(latitude, longitude, speed, direction, map,p_m_id2) {
	...
	if (!isNaN(latitude) && !isNaN(longitude)) {
		...
		contentString = '<div style="font-size: 12px;">' + // Thay đổi kích thước font
		'<strong>Wind Speed:</strong> ' + speed + ' m/s</div>' +
		'<div style="font-size: 12px;">' + // Thay đổi kích thước font
		'<strong>m_id:</strong> ' + p_m_id2 + '</div>' +
		'<div style="font-size: 12px;">' + // Thay đổi kích thước font
		'<strong>Wind Direction:</strong> ' + direction + '°</div>';
